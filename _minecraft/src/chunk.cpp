...
float NYChunk::_WorldUV[CHUNK_SIZE*CHUNK_SIZE*CHUNK_SIZE*3*4*6];

namespace
{
	// Update vertex, color and normal data for vertex at x, y, z (+1 to reach different points in the cube)
	void SetVertColNorm(float * &ptVert, float * &ptCols, float * &ptNorm, float * &ptUV, int x, int y, int z, NYColor color, const std::uniform_real_distribution<float> &dist,
		NYVert3Df normal, float u, float v)
	{
			...
			for (int z = 0; z < CHUNK_SIZE; z++)
			{
				...
				if (cube._Draw && cube._Type != NYCubeType::CUBE_AIR)
				{
					...
					if (cubeZPrev == nullptr || !cubeZPrev->isSolid())
					{
						...
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z, color, dist, normal, 0, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z, color, dist, normal, 1, 0);
					...
					if (cubeXNext == nullptr || !cubeXNext->isSolid())
					{
						//Second QUAD (X+)
						NYVert3Df normal = NYVert3Df(1, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z, color, dist, normal, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z + 1, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z + 1, color, dist, normal, 0, 1);
						_NbVertices += 4;
					}

					if (cubeXPrev == nullptr || !cubeXPrev->isSolid())
					{
						//Troisieme QUAD (X-)
						NYVert3Df normal = NYVert3Df(-1, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z + 1, color, dist, normal, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z + 1, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z, color, dist, normal, 0, 1);
						_NbVertices += 4;
					}

					if (cubeYNext == nullptr || !cubeYNext->isSolid())
					{
						//Quatrieme QUAD (Y+)
						NYVert3Df normal = NYVert3Df(0, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z + 1, color, dist, normal, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z + 1, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z, color, dist, normal, 0, 1);
						_NbVertices += 4;
					}

					if (cubeYPrev == nullptr || !cubeYPrev->isSolid())
					{
						//Cinquieme QUAD (Y-)
						NYVert3Df normal = NYVert3Df(0, -1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z, color, dist, normal, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z + 1, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z + 1, color, dist, normal, 0, 1);
						_NbVertices += 4;
					}
					...
					if (cubeZNext == nullptr || !cubeZNext->isSolid())
					{
						//Sixieme QUAD (Z+)
						NYVert3Df normal = NYVert3Df(0, 0, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y, z + 1, color, dist, normal, 0, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y, z + 1, color, dist, normal, 1, 0);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x + 1, y + 1, z + 1, color, dist, normal, 1, 1);
						SetVertColNorm(ptVert, ptCols, ptNorm, ptUV, x, y + 1, z + 1, color, dist, normal, 0, 1);
						_NbVertices += 4;
					}
